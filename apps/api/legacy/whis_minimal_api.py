...
try:
    ...
    # Define metrics
    REQUEST_COUNT = Counter('whis_requests_total', 'Total requests', ['endpoint', 'status'])
    REQUEST_DURATION = Histogram('whis_request_duration_seconds', 'Request duration')
    EVENTS_PROCESSED = Counter('whis_events_total', 'Total events processed', ['severity'])
    MITRE_TECHNIQUES = Counter('whis_mitre_techniques_total', 'MITRE techniques detected', ['technique'])
